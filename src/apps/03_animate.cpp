...
void animate_frame(Scene* scene) {   
    ...
    for(auto s: scene->surfaces){
        ...
        s->frame=animate_compute_frame(s->animation, scene->animation->time);
        // update the _display_mesh frame if exists
        if(s->_display_mesh!=nullptr){
            s->_display_mesh->frame=s->frame;
        }
    }
}
...
void animate_skin(Scene* scene) {
    ...
    for(auto m : scene->meshes){
        ...
        if(m->skinning==nullptr){
            continue;
        }
        auto skin=m->skinning;
        ...
        for(i=0; i< (m->pos).size(); i++){
            ...
            vec4i bone_ids=skin->bone_ids[i];
            ...
            for(j=0; j<4; j++){
                ...
                // grab bone xform
                mat4f xform=(skin->bone_xforms)[scene->animation->time][bone];
                // accumulate weighted and transformed rest position and normal
                m->pos[i]+=weight*transform_point(xform,(skin->rest_pos[i]));
                m->norm[i]+=weight*transform_vector(xform,(skin->rest_norm[i]));
            }
            normalize(m->norm[i]);
...
void simulate(Scene* scene) {
    ...
    for(Mesh* m  : scene->meshes){
       ...
       if(m->simulation==nullptr){
           continue;
       }
       // compute time per step
       auto time_per_step=(scene->animation->dt)/float(scene->animation->simsteps);

       int step;

       // foreach simulation steps
       for(step=0; step< scene->animation->simsteps; step++){
           // initialize particle forces to zero3f
           for(vec3f p: (m->simulation->force)){
               p=zero3f;
           }
           ...
           int p;
           for(p=0; p<(m->simulation->init_pos).size(); p++){

                // compute force of gravity
               vec3f gravity=(m->simulation->mass)[p]*(scene->animation->gravity);
                // compute force of wind
                // accumulate sum of forces on particle
               (m->simulation->force)[p]=gravity;
           }
           ...
           for(p=0; p < (m->simulation->init_pos).size(); p++){
                ...
                // if pinned, skip
               if(m->simulation->pinned[p]){
                   continue;
               }
                // compute acceleration
               vec3f acc=(m->simulation->force)[p]/float((m->simulation->mass)[p]);
                // update velocity and positions using Euler's method
               vec3f vel=(m->simulation->vel)[p] + acc*time_per_step;
               (m->simulation->vel)[p]=vel;
               (m->pos)[p]=(m->pos)[p] + vel*time_per_step+ acc*time_per_step*time_per_step/2;
                // for each surface, check for collision
               for(Surface* s: scene->surfaces){
                    // compute inside tests...
                   bool inside=false;
                   vec3f collision_pos;
                   vec3f collision_norm;
                    // if quad
                   if(s->isquad){
                        // perform inside test
                        if(dot(m->pos[p]-s->frame.o, s->frame.z)<0){
                            inside=true;
                            // if inside, compute a collision position and normal
                            collision_pos=vec3f(m->pos[p].x, m->pos[p].y, -(m->pos)[p].z)-.01*(m->simulation->vel[p]);
                            collision_norm=normalize(s->frame.z);
                        }
                   }
                    // if sphere
                   else{
                        // inside test
                       float distance=length((m->pos)[p]-s->frame.o);
                       if(distance< s->radius){
                            // if inside, compute a collision position and normal
                           inside=true;
                           collision_norm=normalize(m->pos[p]-s->frame.o);
                           collision_pos=s->frame.o+normalize(collision_norm)*s->radius-.01*(m->simulation->vel[p]);
                   ...
                   if(inside){
                       ...
                       m->pos[p]=collision_pos;
                        // update velocity (particle bounces), taking into account loss of kinetic energy
                       vec3f norm_vel=collision_norm*(dot((m->simulation->vel)[p], collision_norm));
                       vec3f p_vel=(m->simulation->vel)[p]-norm_vel;
                       (m->simulation->vel)[p]=-norm_vel*(1.0-(scene->animation->bounce_dump).y)+p_vel*(1.0-(scene->animation->bounce_dump).x);
                   }
               }
           }        
       }
       if((m->triangle).size()>0 || (m->quad).size()>0){
           smooth_normals(m);
       }
    }
}
